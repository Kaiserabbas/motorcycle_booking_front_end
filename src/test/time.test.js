...
describe('Testing the Time Validator functionality', () => {
  ...
  test('should return valid==true for date1 <= date2 ', () => {
    const today = new Date();
    const from = `${today.getDate()}-${1 + today.getMonth()}-${today.getFullYear()}`;
    const to = `${today.getDate() + 2}-${1 + today.getMonth()}-${today.getFullYear()}`;
  ...
  test('should return false when  from > to ', () => {
    const today = new Date();
    const from = `${today.getDate() + 20}-${1 + today.getMonth()}-${today.getFullYear()}`;
    const to = `${today.getDate() + 2}-${1 + today.getMonth()}-${today.getFullYear()}`;
    const valid = validateTime2(from, to);
    expect(valid).toBe(false);
  });
